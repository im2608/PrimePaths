...
#         self.dp = np.ones((len(self.X), 2**len(self.X)))*-1
    ...
    # 返回值为从 node 出发，遍历完 future_set 中所有的节点之后的距离 
    def solve(self,node,future_sets):
        su = self.transfer(future_sets) # su 为 future_sets 所对应的二进制标志
#         if (self.dp[node][su] != -1):
#             return self.dp[node][su]
        if (node in self.dp and su in self.dp[node]):
            return self.dp[node][su]
...
#             return self.X[node][self.start_node]
        ...
        if (node not in self.array):
            self.array[node] = {}            
        self.array[node][su] = next_one # 表示从 node 出发，第一个遍历  future_set 中的那个节点
...
#         self.dp[node][su] = d
    ...
    def get_path(self):
        ...
        while len(lists) > 0:
            ...
            if (start in self.array and m in self.array[start]):
